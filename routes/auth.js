...
router.post('/login', [
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT id, username, email, password_hash, student_id, diploma, grade FROM users WHERE username = $1',
      [username]
    );
...
router.get('/google/callback', async (req, res) => {
  ...
  try {
    ...
    if (tokens.error) {
      console.error('Google OAuth 오류:', tokens);
      return res.render('login', { 
    ...
    if (!email.endsWith('@cnsa.hs.kr')) {
      return res.render('login', { 
        error: '학교 계정(@cnsa.hs.kr)만 사용할 수 있습니다.',
        csrfToken: req.csrfToken()
      });
    }
    ...
    const userResult = await pool.query(
      'SELECT id, username, email, student_id, diploma, grade FROM users WHERE google_id = $1 OR email = $2',
      [googleId, email]
    );
    ...
    res.redirect('/auth/complete-registration');
  } catch (error) {
...
router.post('/complete-registration', [
  ...
  body('student_id').trim().notEmpty().withMessage('학번을 입력하세요'),
  body('diploma').trim().notEmpty().withMessage('디플로마를 선택하세요')
], async (req, res) => {
  if (!req.session.pendingGoogleAuth) {
    return res.redirect('/auth/register');
  }
  ...
  try {
    ...
    // PVP 통계 초기화
    await pool.query(
      'INSERT INTO pvp_stats (user_id) VALUES ($1)',
      [user.id]
    );
...
router.post('/register', [
  ...
  try {
    ...
    if (duplicateCheck.rows.length > 0) {
      return res.render('register', { 
        error: '이미 사용 중인 아이디, 이메일 또는 학번입니다.',
        csrfToken: req.csrfToken()
      });
    }
    ...
    // PVP 통계 초기화
    await pool.query('INSERT INTO pvp_stats (user_id) VALUES ($1)', [user.id]);
